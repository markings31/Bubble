        ...
        Remain.getOnlinePlayers().forEach(loopPlayer -> {
            ...
            if (message.toLowerCase().contains("@" + playerName.toLowerCase())
                ...
                if (cache.isMentionToastStatus())
                    Notification.send(new Tuple<>(null, loopPlayer),
                            NotificationTypes.TOAST.getLabel(), Localization.NotificationMessages.MENTIONED_MESSAGE, CompMaterial.PAPER.toString());
